      ...
      skill: {
        ...
        "PSsp_linggan": {
          ...
          filter: function (event, player) {
            ...
            return player.storage.PSsp_sucai && (!player.storage.PSsp_sucai[0].includes(get.suit(event.card)) || !player.storage.PSsp_sucai[1].includes(get.number(event.card)) || !player.storage.PSsp_sucai[2].includes(get.name(event.card)));
          },
          usable: 1,
          prompt2: function (event, player) {
            let str = '获得';
            if (!player.storage.PSsp_sucai[0].includes(get.suit(event.card))) str += `一枚花色为${get.suitTranslation(get.suit(event.card))}的徽章，`;
            if (!player.storage.PSsp_sucai[1].includes(get.number(event.card))) str += `一枚点数为<span class = "greentext">${get.translation(get.number(event.card))}</span>的徽章，`;
            if (!player.storage.PSsp_sucai[2].includes(get.name(event.card))) str += `一枚牌名为<span class = "greentext">${get.translation(get.name(event.card))}</span>的徽章。`;
            if (str.at(-1) === '，') str = str.slice(0, -1) + '。';
            return str;
          },
          content: function () {
            if (!player.storage.PSsp_sucai[0].includes(get.suit(trigger.card))) player.storage.PSsp_sucai[0].add(get.suit(trigger.card));
            if (!player.storage.PSsp_sucai[1].includes(get.number(trigger.card))) player.storage.PSsp_sucai[1].add(get.number(trigger.card));
            if (!player.storage.PSsp_sucai[2].includes(get.name(trigger.card))) player.storage.PSsp_sucai[2].add(get.name(trigger.card));
        ...
        "PSsp_chongzu": {
          ...
          subSkill: {
            ...
            guess: {
              ...
              firstDo: true,
              filter: function (event, player) {
                return ['basic', 'trick'].includes(get.type(event.card)) && event.targets.filter(target => target.isAlive() && target.isIn() && target !== player).length;
              },
              content: function () {
                'step 0'
